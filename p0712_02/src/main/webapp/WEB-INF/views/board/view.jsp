...
<section>
      ...
      <tr>
        <td colspan="3"><strong>다음글</strong> <span class="separator">|</span>
	        <c:if test="${map.nextDto.bno != null}">
	        	<a href="/board/view?bno=${map.nextDto.bno}&page=${page}"> ${map.nextDto.btitle}</a>
	        </c:if>
	        <c:if test="${map.nextDto.bno == null}">
	        	 다음글이 없습니다.
	        </c:if>
      ...
      <tr>
        <td colspan="3"><strong>이전글</strong> <span class="separator">|</span>
	        <c:if test="${map.prevDto.bno != null}">
		        <a href="/board/view?bno=${map.prevDto.bno}&page=${page}"> ${map.prevDto.btitle}</a>
	        </c:if>
	        <c:if test="${map.prevDto.bno == null}">
		        이전글이 없습니다.
	        </c:if>
    ...
    <a href="/board/update?bno=${map.boardDto.bno}&page=${page}"><div class="list">수정</div></a>
